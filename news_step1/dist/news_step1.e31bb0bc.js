...
var newsList = JSON.parse(ajax.response); //6번에서 요청했을 때 서버측의 응답을 받아오는 속성이 response
console.log(newsList); //서버측에서 보낸 응답을 출력해 보기 - Array로 변환하여 출력해줌
var ul = document.createElement("ul"); //DOM API 가지고 태그를 만드니까 직관적이지 않다. DOM Tree가 그려지지 않음

//해시값이 변경될 때 - 인터셉트 - @id -> Array 에 담긴 id값으로 변경
addEventListener("hashchange", function () {
  // console.log('해시변경?!??!?');
  console.log(location.hash); //#38019991
  var id = location.hash.substring(1); //파라미터에 1을 주어서 #을 제외시킨다.
  console.log(id);
  ajax.open("GET", CONTENT_URL.replace("@id", id), false);
  ajax.send();
  var ncontent = JSON.parse(ajax.response);
...
for (var i = 0; i < 5; i++) {
  ...
  a.href = "#".concat(newsList[i].id);
  // console.log(newsList[i].title);
  a.innerHTML = "".concat(newsList[i].title, "(click ").concat(newsList[i].comments_count, ")");
...
//# sourceMappingURL=/news_step1.e31bb0bc.js.map